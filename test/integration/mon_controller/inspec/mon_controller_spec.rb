describe file('/etc/nagios/nrpe.d/check_cinder_api.cfg') do
  its('content') do
    should match(%r{command\[check_cinder_api\]=/usr/lib64/nagios/plugins/check_http -I 127.0.0.1 -p 8776})
  end
end

describe file('/etc/nagios/nrpe.d/check_heat_api.cfg') do
  its('content') do
    should match %r{command\[check_heat_api\]=/usr/lib64/nagios/plugins/check_http -I 127.0.0.1 -p 8004}
  end
end

describe file('/etc/nagios/nrpe.d/check_keystone_api.cfg') do
  its('content') do
    should match %r{command\[check_keystone_api\]=/usr/lib64/nagios/plugins/check_http --ssl -I 127.0.0.1 -p 5000}
  end
end

describe file('/etc/nagios/nrpe.d/check_neutron_api.cfg') do
  its('content') do
    should match %r{command\[check_neutron_api\]=/usr/lib64/nagios/plugins/check_http -I 127.0.0.1 -p 9696}
  end
end

describe file('/etc/nagios/nrpe.d/check_nova_api.cfg') do
  its('content') do
    should match %r{command\[check_nova_api\]=/usr/lib64/nagios/plugins/check_http -I 127.0.0.1 -p 8774}
  end
end

describe file('/etc/nagios/nrpe.d/check_nova_placement_api.cfg') do
  its('content') do
    should match %r{command\[check_nova_placement_api\]=/usr/lib64/nagios/plugins/check_http -I 127.0.0.1 -p 8778}
  end
end

describe file('/etc/nagios/nrpe.d/check_novnc.cfg') do
  its('content') do
    should match %r{command\[check_novnc\]=/usr/lib64/nagios/plugins/check_http --ssl -I 127.0.0.1 -p 6080}
  end
end
