...
control 'compute-controller' do
  ...
  describe ini('/etc/placement/placement.conf') do
    ...
    its('placement_database.connection') { should cmp "mysql+pymysql://placement:placement@#{db_endpoint}:3306/placement_x86" }
  end
  ...
  describe ini('/etc/nova/nova.conf') do
    ...
    its('api_database.connection') { should cmp "mysql+pymysql://nova:nova@#{db_endpoint}:3306/nova_api_x86" }
    its('cache.memcache_servers') { should cmp 'controller.example.com:11211' }
    its('database.connection') { should cmp "mysql+pymysql://nova:nova@#{db_endpoint}:3306/nova_x86" }
