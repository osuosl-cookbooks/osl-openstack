...
describe 'osl-openstack::controller' do
  ...
  it do
    expect(chef_run).to install_apache2_install('openstack').with(
      modules: %w(status alias auth_basic authn_core authn_file authz_core authz_groupfile authz_host authz_user autoindex deflate dir env negotiation setenvif log_config logio unixd systemd mime),
      mpm_conf: {
